...
begin
  ...
  StrBreakApart(Text,'/',StringList);
  Text:=StringList.strings[0];
  if StrToInt(Text)=127 then begin
...
begin
  ...
  if not TakeTailor and not TakeBS then begin
    AddToSystemJournal('Atenção erro! Pelo menos um dos parâmetros TakeTailor ou TakeBS deve ser True. O script foi interrompido.')
    Halt;
  end;
  myProfiles:=['tailor']; 
