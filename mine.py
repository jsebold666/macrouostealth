...
config[CharName()] = {
    ...
    "discord_webhook_url": "https://discordapp.com/api/webhooks/778417879288381440/LN7_fA4q3YIMHU8KXIUiYRjOeZwliU-cvPblZaMqcAlj8_zkvwo5e9K-6-VTKIU8SLwJ",
    "discord_webhook_afk_url": "https://discordapp.com/api/webhooks/778417879288381440/LN7_fA4q3YIMHU8KXIUiYRjOeZwliU-cvPblZaMqcAlj8_zkvwo5e9K-6-VTKIU8SLwJ",
    "auto_reequip_set": True,
    "auto_refill_tithing_points": True,
    "minimum_tithing_points": 3000,
    # runebook names
    "home_runebook": "Home",
    "ore_runebooks": ["Ore1", "Ore2", "Ore3", "Ore4"],
    "travel_spell": "",
...
def upload():
    ...
    if (
        ...
        # getitems(types.TINKER_TOOLS, 1, 1, storage)
...
def check_tinker_tools_on_storage(minAmount):
    if (
        ...
        amount_of_tinker_tools_in_storage = utils.count(
            types.TINKER_TOOLS, ORE_STORAGE_CONTAINER
        )
        utils.debug(
            ...
            for runebook in MINE_RUNEBOOKS:
                ...
                for runenumber in range(1, 17):
                    ...
                    while (
                        ...
                        # if droping stuff allows lowers weight bellow the limit, we can mine again, so break
                        drop_special_resources_on_ground_maybe()
                        # if Weight() <= WEIGHT_LIMIT:
                        #     utils.debug(
