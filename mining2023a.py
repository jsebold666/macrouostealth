...
def remount():
    if PET:
        if findpet(PET):
            print(f"vai buscar a distanci {PET}")
            if (
                IsObjectExists(PET)
                and GetDistance(PET) <= 1
            ):
                UOSay("all guard me")
                Wait(1000)
                ...
                print(f"disconnect for {PET}")
                Disconnect()
                print(f"connect for pet {PET}")
                Wait(5000)
                Connect()
                while not Connected():
                    Wait(5000)
        ...
        for ore in ores_in_backpack:
            ...
            while IsObjectExists(ore):
                if GetQuantity(ore) <= 1:
                    utils.debug("Ore pile is only 1. Abort smelting?sss")
                    break
                UseObject(ore)
                WaitForTarget(3000)
...
def upload():
    ...
    if (
        ...
        check_tinker_tools_on_storage(1)
        ##make_tinker_tools(7)
...
def check_tinker_tools_on_storage(minAmount):
    if (
        ...
        amount_of_tinker_tools_in_storage = GetQuantity(FindTypeEx(types.TINKER_TOOLS, 0, ORE_STORAGE_CONTAINER, True))
        ...
        #         make_tinker_toolss(1)
        #     else:
        #         getitems(types.TINKER_TOOLS, 1, 1, ORE_STORAGE_CONTAINER)
        utils.debug("check tinker tools on storage")
        if GetQuantity(FindTypeEx(types.TINKER_TOOLS, 0, ORE_STORAGE_CONTAINER, True)) < 1:
            utils.debug("Low on tinker on storage! Making it")
            Tinkering(
                types.TINKER_TOOLS, minAmount + 1, TINKERMENU_TINKERTOOLS
...
def make_tinker_tools(min_amount=1):
    if GetQuantity(FindTypeEx(types.TINKER_TOOLS, 0, ORE_STORAGE_CONTAINER, True)) <= 1:
        # check tinker tools
...
def make_tinker_toolss(min_amount=1):
    if GetQuantity(FindTypeEx(types.TINKER_TOOLS, 0, ORE_STORAGE_CONTAINER, True)) <= 1:
        # check tinker tools
        amount_of_tinker_tools_in_backpack = GetQuantity(
            FindTypeEx(types.TINKER_TOOLS, 0x0000, Backpack(), False)
        )
        if amount_of_tinker_tools_in_backpack < min_amount:
            utils.debug("Making tinker tools")
            return Tinkering(types.TINKER_TOOLS, 1, TINKERMENU_TINKERTOOLS)
            ...
            if current_tithing_points < MINIMUM_TITHING_POINTS:
                if AUTO_REFILL_TITHING_POINTS:
                    ...
                    if Gold() > 0:
                        ...
                        send_discord_message(
                                DISCORD_WEBHOOK_URL,
                                f"**MINER GO TO ANK** @everyone MINER GO TO ANK **{ProfileName()} ({char_name})**",
                        )
